...
def main(keyWord, nWords, since, getCaps):
    ...
    if getCaps == False:
        ...
        plt.savefig(f'word-clouds/trump-word-cloud-{nWords}-words-{str(keyWord)}-since-{since}.png')
        plt.show()
        ...
        plt.savefig(f'word-clouds/trump-word-cloud-{nWords}-words-{str(keyWord)}-since-{since}-CAPS.png')
        plt.show()
        print('Word Clouds successfully printed to directory.')
